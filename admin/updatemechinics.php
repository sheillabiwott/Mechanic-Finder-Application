...
if(isset($_POST['updatemechanic'])){
							...
							$status = $_POST['status'];
							$query="UPDATE professional SET mechanic_Fullname='$name',mechanic_address='$adress ',mechanic_city='$city',mechanic_contact='$contact',mechanic_email='$email',experience='$experience',rate_per_hour='$hourlyrate',status='$status' WHERE mechanic_id='$mach_id'";
							$dataresult=mysqli_query($connection,$query);
							if($dataresult){
								$msg="Machanic Updated";
								echo("<script>alert('".$msg."'); window.location.replace('manageprofessional.php');</script>");
